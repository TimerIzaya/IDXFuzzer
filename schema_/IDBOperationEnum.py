...
# ===== collected lists (append-only) =====

def _as_op(x):
    # 你的定义行末尾加了逗号，变量会变成单元素 tuple；这里统一解包成真正的实例
    return x[0] if isinstance(x, tuple) and len(x) == 1 else x

def _collect_ops(kind_name: str):
    ops = []
    for v in globals().values():
        if v is None:
            continue
        obj = _as_op(v)
        # 只收集实例，不收集类本身；并且限定三种 Operation 类型
        if hasattr(obj, "interfaceName") and hasattr(obj, "name") \
           and hasattr(obj, "depOps") and hasattr(obj, "incOps") \
           and type(obj).__name__ == kind_name:
            ops.append(obj)
    # 稳定顺序，方便 diff / 调试
    ops.sort(key=lambda o: (o.interfaceName, o.name))
    return ops

MethodOperations: List[MethodOperation]       = _collect_ops("MethodOperation")
EventOperations: List[EventOperation]         = _collect_ops("EventOperation")
PropertyOperations: List[PropertyOperation]   = _collect_ops("PropertyOperation")
ALL_OPERATIONS: List[object]                  = [*MethodOperations, *EventOperations, *PropertyOperations]
