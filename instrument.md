...
**content/browser/indexed_db** 与 **components/services/storage** 都属于“浏览器侧后端”，在 `content_shell` 中默认 **运行于同一个 Browser 进程**；因此这两部分的插桩数据最终汇聚成 **同一份 Browser 侧覆盖率文件**。

Blink 的 IndexedDB 模块运行在 **单独的 Renderer 进程** 中，因此会导出 **独立的一份 Renderer 覆盖率文件**。

另外解释一下，blink是渲染引擎框架，是一套代码库，render是他妈一个进程，负责渲染的
